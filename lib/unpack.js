...
const pruneCache = (cache, abs) => {
  ...
  for (const path of cache.keys()) {
    ...
    if (plower === abs || plower.indexOf(abs + '/') === 0)
      cache.delete(path)
  }
}
...
class Unpack extends Parser {
  ...
  [CHECKPATH] (entry) {
    ...
    if (!this.preservePaths) {
      ...
      // Strip the root off of the path *before* looking for '..' parts.
      // Absolutes on posix are also absolutes on win32, so testing with the
      // win32 parser catches both.  It also catches drive-relative paths
      // like 'c:../foo' or 'c:foo', which are not absolute, but whose root
      // would otherwise make path.resolve() escape the extraction target.
      const rootAndPath = stripAbsolutePath(p)
      const root = rootAndPath[0]
      const stripped = rootAndPath[1]

      // check for '..' on the *stripped* path, so that 'c:..' and
      // '/../foo' are caught along with the plain 'a/../b' case.
      if (stripped.split('/').indexOf('..') !== -1) {
